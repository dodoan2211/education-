...
import { BookOpen, Calendar, MonitorPlay, Presentation, ArrowRight, ArrowRightCircle, Sparkles, Loader2, Settings, LogOut, ChevronDown, FlaskConical, Code, Cpu, BrainCircuit, Share2, FileText, Heart, QrCode } from "lucide-react";
...
const ACCOUNT_NAME = "DO VAN DOAN";
const BANK = "MB";
const ACCOUNT_NO = "9666989889";

function DonateQR({ amount = "50000", compact = false }: { amount?: string; compact?: boolean }) {
  const qrUrl = `https://img.vietqr.io/image/${BANK}-${ACCOUNT_NO}-compact2.png?amount=${amount}&addInfo=${encodeURIComponent("DONATE EDUCREATE")}&accountName=${encodeURIComponent(ACCOUNT_NAME)}`;
  if (compact) {
    return (
      <div className="flex flex-col items-center gap-2">
        <img src={qrUrl} alt="Donate QR" className="w-28 h-28 object-contain rounded-xl border border-slate-100" />
        <p className="text-[10px] text-slate-500 font-medium text-center">Ủng hộ dự án</p>
      </div>
    );
  }
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col items-center gap-4 max-w-xs w-full">
      <div className="flex items-center gap-2 text-rose-600">
        <Heart className="w-5 h-5 fill-rose-500" />
        <span className="font-bold text-base text-slate-900">Ủng hộ EduCreate</span>
      </div>
      <img src={qrUrl} alt="Donate QR" className="w-44 h-44 object-contain rounded-xl border border-slate-100" />
      <div className="text-center space-y-1 w-full">
        <div className="grid grid-cols-2 gap-x-3 text-xs text-left bg-slate-50 rounded-xl p-3 border border-slate-100">
          <span className="text-slate-500 font-medium">Ngân hàng:</span>
          <span className="font-bold text-slate-800">MB BANK</span>
          <span className="text-slate-500 font-medium">Tài khoản:</span>
          <span className="font-bold text-slate-800">{ACCOUNT_NO}</span>
          <span className="text-slate-500 font-medium">Chủ TK:</span>
          <span className="font-bold text-slate-800 uppercase">{ACCOUNT_NAME}</span>
        </div>
        <p className="text-[11px] text-slate-400 leading-snug mt-2">Mọi khoản ủng hộ đều giúp duy trì và phát triển EduCreate cho cộng đồng giáo viên.</p>
      </div>
    </div>
  );
}
...
export default function Landing() {
  const [activeTab, setActiveTab] = useState<'home' | 'community' | 'share' | 'donate' | 'stem' | 'ai' | 'arduino'>('home');
  ...
  useEffect(() => {
    ...
    if (params.get("post")) setActiveTab('community');
  }, []);

  useEffect(() => {
    if (activeTab === "share") fetchSharedResources();
  }, [activeTab]);
  ...
  const typeLabel: Record<string, string> = {
    lesson_plan: "Giáo án", plan: "Kế hoạch", digital: "Chuyển đổi số",
    video: "Bài giảng Video", infographic: "Infographic",
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden md:flex items-center space-x-6">
              <button onClick={() => setActiveTab('community')} className={`text-sm font-medium transition-colors ${activeTab === 'community' ? 'text-blue-600' : 'text-slate-600 hover:text-slate-900'}`}>
                Cộng đồng
              </button>
              <button onClick={() => setActiveTab('share')} className={`text-sm font-medium transition-colors ${activeTab === 'share' ? 'text-blue-600' : 'text-slate-600 hover:text-slate-900'}`}>
                Chia sẻ
              </button>
              <button onClick={() => setActiveTab('donate')} className={`text-sm font-medium transition-colors flex items-center gap-1 ${activeTab === 'donate' ? 'text-rose-600' : 'text-slate-600 hover:text-rose-600'}`}>
                <Heart className="w-3.5 h-3.5" /> Ủng hộ
              </button>
              {user && (
                <Link to="/dashboard" className="text-sm font-bold text-slate-900 hover:text-blue-600 transition-colors">
                  Trình quản lý
                </Link>
              )}
            </div>
            ...
            <div className="flex items-center space-x-4">
              {!user ? (
                ...
                <div className="relative">
                  <div className="flex items-center gap-2 cursor-pointer group hover:bg-slate-50 px-2 py-1.5 rounded-lg transition-colors" onClick={() => setShowUserMenu(!showUserMenu)}>
                    {userProfile?.avatar ? (
                  ...
                  {showUserMenu && (
                    <>
                      ...
                      <div className="absolute top-full right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-slate-200 py-1 z-50 animate-fade-in origin-top-right overflow-hidden">
                        ...
                        <button onClick={() => { setShowUserMenu(false); handleLogout(); }} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 transition-colors text-left">
      ...
      <main className="flex-grow">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
                <button onClick={() => setActiveTab('stem')} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-lg hover:border-rose-200 transition-all group text-left cursor-pointer relative overflow-hidden">
                  ...
                  <div className="flex items-center text-xs font-bold text-rose-600 opacity-0 group-hover:opacity-100 transition-opacity">Xem chi tiết <ArrowRight className="w-3 h-3 ml-1" /></div>
                </button>
                ...
                <Link to="/tool/lesson_plan" className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-lg hover:border-blue-200 transition-all group text-left cursor-pointer relative">
                  ...
                  <div className="flex items-center text-xs font-bold text-blue-600 opacity-0 group-hover:opacity-100 transition-opacity">Mở công cụ <ArrowRight className="w-3 h-3 ml-1" /></div>
                </Link>
                ...
                <button onClick={() => setActiveTab('arduino')} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-lg hover:border-amber-200 transition-all group text-left cursor-pointer relative overflow-hidden">
                  ...
                  <div className="flex items-center text-xs font-bold text-amber-600 opacity-0 group-hover:opacity-100 transition-opacity">Xem chi tiết <ArrowRight className="w-3 h-3 ml-1" /></div>
                </button>
                ...
                <button onClick={() => setActiveTab('ai')} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-lg hover:border-indigo-200 transition-all group text-left cursor-pointer relative overflow-hidden">
                  ...
                  <div className="flex items-center text-xs font-bold text-indigo-600 opacity-0 group-hover:opacity-100 transition-opacity">Xem chi tiết <ArrowRight className="w-3 h-3 ml-1" /></div>
                </button>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                ...
                <button onClick={() => setActiveTab('donate')} className="w-full sm:w-auto px-8 py-4 text-base font-semibold bg-rose-50 text-rose-700 border border-rose-200 rounded-lg hover:bg-rose-100 transition-all flex items-center justify-center gap-2 cursor-pointer">
                  <Heart className="w-4 h-4 fill-rose-500" /> Ủng hộ dự án
                </button>
              </div>
            </div>
          ...
          <section className="py-12 bg-slate-50 min-h-[calc(100vh-160px)]">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10 pb-6 border-b border-slate-200">
                ...
                {user && (
                  <Link to="/dashboard" className="w-full md:w-auto px-5 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold transition-all shadow-md shadow-blue-100 flex items-center justify-center gap-2 shrink-0">
                    <Sparkles className="w-5 h-5" /> Tạo & chia sẻ tài liệu
                  </Link>
                )}
              </div>
                    ...
                    <div key={item.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col hover:shadow-md transition-shadow">
                      <div className="p-6 flex-1">
                        ...
                        <div className="flex items-center gap-2 text-xs text-slate-500 pt-2 border-t border-slate-50">
                          ...
                          <span>{item.workplace || "Giáo viên"}</span>
                        </div>
                      </div>
        ...
        {/* Donate Tab */}
        {activeTab === 'donate' && (
          <section className="py-16 bg-slate-50 min-h-[calc(100vh-160px)]">
            <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-rose-100 text-rose-700 text-xs font-semibold mb-4">
                <Heart className="w-3.5 h-3.5 fill-rose-500" /> Ủng hộ dự án giáo dục
              </div>
              <h2 className="text-3xl font-bold text-slate-900 mb-4">Ủng hộ EduCreate</h2>
              <p className="text-base text-slate-600 max-w-xl mx-auto mb-10 leading-relaxed">
                EduCreate là dự án phi lợi nhuận, hoàn toàn miễn phí cho giáo viên. Nếu công cụ này hữu ích với thầy cô, hãy ủng hộ để chúng tôi tiếp tục duy trì và phát triển nền tảng.
              </p>
              ...
              <div className="flex flex-col md:flex-row gap-8 items-start justify-center">
                ...
                <div className="flex-1 text-left space-y-4 max-w-sm">
                  <h3 className="font-bold text-slate-900 text-lg">Khoản ủng hộ của bạn giúp:</h3>
                  <ul className="space-y-3">
                    {[
                      "Duy trì server và chi phí vận hành hàng tháng",
                      "Phát triển tính năng AI mới cho giáo viên",
                      "Xây dựng thư viện tài liệu chia sẻ cộng đồng",
                      "Giữ EduCreate miễn phí mãi mãi cho mọi giáo viên",
                    ].map((item, i) => (
                  ...
                  <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-xs text-blue-800 mt-4 leading-relaxed">
                    <span className="font-bold block mb-1">Cảm ơn thầy cô!</span>
                    Quét mã QR bên cạnh bằng ứng dụng ngân hàng để chuyển khoản. Mọi khoản ủng hộ dù nhỏ đều rất có ý nghĩa với chúng tôi.
                  </div>
                </div>
              </div>
            </div>
      ...
      <footer className="bg-white border-t border-slate-200 py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <button onClick={() => setActiveTab('donate')} className="flex items-center gap-1.5 text-sm text-rose-600 hover:text-rose-700 font-medium transition-colors">
            <Heart className="w-4 h-4 fill-rose-500" /> Ủng hộ dự án
          </button>
